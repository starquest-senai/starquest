...
if (isset($_SESSION['usuario']['email'])){
   ...
   // $verifica_usuario_bd = mysqli_query($conexao, "SELECT * FROM jogadores WHERE usuario = '{$usuario}'") or die("ERRO no cadastro verifica_nome_bd");
    ...
    if (mysqli_num_rows($verifica_email_bd) > 0){
        echo "UPDATE jogadores SET senha = '{$senha}' WHERE usuario = '{$usuario_sessao}' ";
        php_update( "UPDATE jogadores SET senha = '{$senha}' WHERE usuario = '{$usuario_sessao}' ");
        ...
        var_dump($_SESSION['usuario']["senha"]);

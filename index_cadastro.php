...
<head>
    ...
    <script>
        function valida_checkbox() {
            var confirmar = document.getElementById("termosUso");
            if (confirmar.checked == true){
                return true;
            } else {
                $('#valide_checkbox').modal('show');
                return false;
            }
        }
        ...
        function valida_senha (input){
            if (input.value != document.getElementById('senha').value) {
                input.setCustomValidity('As senhas não conferem, repita corretamente.');
            } else {
...
<body>
    ...
    <form class="form-signin" action="includes/index_cadastro.php" method="post" onsubmit="return valida_checkbox()">
        ...
        <div class="form-row">
            <div class="form-group col-md-6">
                ...
                <input type="text" id="nome_completo" name="nome" class="form-control" placeholder="Nome completo" <?php if (isset($_GET['nome'])) { ?> value="<?php echo $_GET['nome'];}?>" required onsubmit="this.value=''">
            </div>
            <div class="form-group col-md-6">
                <label for="inputEmail4" style="font-weight: bold">Email</label>
                <input type="email" class="form-control" id="inputEmail4" name="email" placeholder="meu@email.com" <?php if (isset($_GET['email'])) { ?> value="<?php echo $_GET['email'];}?>" required>
            </div>
            <div class="form-group col-md-6">
                <label for="usuario" style="font-weight: bold">Nome de usuário</label><br>
                <div class="input-group-prepend">
                    <span class="input-group-text" id="basic-addon1">@</span>
                    <input type="text" class="form-control" id="usuario" name="usuario" placeholder="Usuário" aria-label="Username" aria-describedby="basic-addon1" <?php if (isset($_GET['usuario'])) { ?> value="<?php echo $_GET['usuario'];}?>" required>
                </div>
            </div>
            <div class="form-group col-md-6">
                <label for="data_nascimento" style="font-weight: bold">Data de nascimento</label><br>
                <input class="form-group col-md-6" id="data_nascimento" name="data_nasc" type="Date" <?php if (isset($_GET['data_nasc'])) { ?> value="<?php echo $_GET['data_nasc'];}?>" required/> <br/>
            </div>
            <div class="form-group col-md-6">
            ...
            <div class="form-group col-md-6">
                <label for="senha_confirma" style="font-weight: bold">Repita Senha</label>
                <input type="password" class="form-control" id="senha_confirma" name="repeteSenha" placeholder="Repita Senha" oninput="valida_senha(this)" required>
            </div>
            <div class="input-group mb-6">
                <label class="termos_uso">Para ler os &laquo;Termos de Uso&raquo; dê um click nesse <a href="#modal_termos_uso" data-toggle="modal">link</a>.</label>
            </div>
            <div id="check_termos_uso" class="input-group mb-6">
                <input type="checkbox" id="termosUso" aria-label="Checkbox for following text input" value="checked">
                <label for="termosUso">&nbsp; Confirmo que lí os termos de uso. </label>
            </div>

            <div class="row">
                <div class="col" style="width: 500px">
                    <button class="btn btn-sm btn-secondary btn-block" id="botao_cadastro" type="button" onclick="window.location.href='index.php'">Cancelar</button>
                </div>
                <div class="col" style="width: 500px">
                    <button class="btn btn-sm btn-primary btn-block" id="botao_cadastro" type="submit">Cadastrar</button>
                </div>
            </div>
    ...
    <!-- MODAL MENSAGEM ERRO CHECKBOX TERMOS DE USO -->
    <div class="modal fade" id="valide_checkbox" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel" style="color: #000000;">Termos de Uso</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" style="color: #000000; text-align: center">
                    <p>Para continuar você deve confirmar que leu os termos de uso</p>
                </div>
    ...
    <!--TERMOS DE USO -->
    <div class="modal fade bd-example-modal-lg" id="modal_termos_uso" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body" style="color: #000000">
                    <h1 style="text-align: center; font-size: 20pt">TERMOS DE SERVIÇO</h1>

                    <p style="text-align: center; font-size: 15pt">----</p>

                    <h3>VISÃO GERAL</h3>

                    <p>Esse site é operado pelo StarQuest. Em todo o site, os termos “nós”, “nos” e “nosso” se referem ao StarQuest. O StarQuest proporciona esse site, incluindo todas as informações, ferramentas e serviços disponíveis deste site para você, o usuário, com a condição da sua aceitação de todos os termos, condições, políticas e avisos declarados aqui.</p>
                    ...
                    <p>Esses Termos de serviço e quaisquer acordos separados em que nós lhe fornecemos os Serviços devem ser regidos e interpretados de acordo com as leis de R. Arno Waldemar Dohler, 957, Joinville, SC, 89219-510, Brazil.</p>
                    ...
                    <p>As perguntas sobre os Termos de serviço devem ser enviadas para nós através do starquest2018.senai@gmail.com.</p>
                </div>
                <div class="modal-footer">
